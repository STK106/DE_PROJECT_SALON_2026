          ...
          <CardContent className="space-y-4">
            {cart.items.map((item) => (
              <div key={item.product_id} className="flex items-center gap-4 rounded-lg border p-4">
                {item.image_urls?.[0] ? (
                  <img src={resolveMediaUrl(item.image_urls[0])} alt={item.name} className="h-16 w-16 rounded-md object-cover" />
                ) : (
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleCheckout} className="space-y-3">
              <Input placeholder="Full name" value={form.shipping_name} onChange={(e) => setForm({ ...form, shipping_name: e.target.value })} />
